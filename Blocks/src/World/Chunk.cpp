...
Chunk::Chunk(BlocksEngine::Actor& actor, const World& world, const ChunkCoords coords)
    : Component{actor},
      world_{world},
      coords_{coords}
{
            ...
            for (int j = 0; j < dimensions[v]; ++j)
            {
                for (int i = 0; i < dimensions[u];)
                {
                    if (int blockId = mask[n]; static_cast<bool>(blockId))
                    {
                        ...
                        int width = 1;
                        while (i + width < dimensions[u] && blockId == mask[static_cast<unsigned long long>(n) + width])
                        {
                            ++width;
                        }
