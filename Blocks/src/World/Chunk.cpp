...
void Chunk::RegenerateMesh(std::function<void()> callback) const
{
    BlocksEngine::DispatchQueue::Background().Async([this, callback = move(callback)]()
    mutable
        {
            ...
            // TODO: Mesh could be captured by a mutable lambda instead of a shared pointer
            GetGame()->MainDispatchQueue().Async([this, mesh, callback = move(callback)]()
            mutable
                {
